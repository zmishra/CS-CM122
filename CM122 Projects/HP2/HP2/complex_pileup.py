...
def generate_donor(ref, aligned_reads):
    ...
    while padded_reads:
        ...
        for padded_read in padded_reads:
            ...
            if score < 45:  # If the alignment isn't good, throw the read back in the set of reads to be aligned.
...
def edit_distance_matrix(ref, donor):
    ...
    return output_matrix, trace_matrix
...
def identify_changes(ref, donor, offset):
    ...
    ref = '${}'.format(ref)
    donor = '${}'.format(donor)
    edit_matrix, trace_matrix = edit_distance_matrix(ref=ref, donor=donor)
    #print(edit_matrix)
    ...
    while current_row > 0 or current_column > 0:
        ...
        if trace_matrix[current_row, current_column] == 2:
            current_row = pvs_row
            current_column = pvs_column
        elif trace_matrix[current_row, current_column] == 3:
            changes.append(['SNP', ref[current_row], donor[current_column], ref_index])
            current_row = pvs_row
            current_column = pvs_column
        elif trace_matrix[current_row, current_column] == 0:
            ...
            current_column = pvs_column
        elif trace_matrix[current_row, current_column] == 1:
...
if __name__ == "__main__":
    ...
    files = os.listdir(input_fn)
    files = [os.path.join(input_fn, f) for f in files]

    with multiprocessing.Pool(processes=12) as pool:
        results = pool.map(generate_pileup, files)
    ...
    for i in range(12):
        print(i)
        snps += results[i][0]
        insertions += results[i][1]
        deletions += results[i][2]
