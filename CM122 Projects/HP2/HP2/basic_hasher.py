...
def hashing_algorithm_mp(paired_end_reads, genome_ht):
    alignments = []
    genome_aligned_reads = []
    chunks = [paired_end_reads[i::12] for i in range(12)]
    with poolcontext(processes=12) as pool:
        result = pool.map(partial(hashing_algorithm, genome_ht=genome_ht), chunks)
    for i in range(12):
        alignments += result[i][0]
        genome_aligned_reads += result[i][1]
    return alignments, genome_aligned_reads
...
if __name__ == "__main__":
    ...
    #hashing_algorithm_mp(reads, genome_hash_table)
    genome_aligned_reads, alignments = hashing_algorithm_mp(reads, genome_hash_table)
    print('writing')
